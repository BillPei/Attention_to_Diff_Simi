...
from common_functions import Conv_with_input_para, Average_Pooling_for_Top, create_conv_para, create_GRU_para, GRU_Tensor3_Input, GRU_Matrix_Input, Matrix_Bit_Shift, GRU_Batch_Tensor_Input, compute_simi_feature_matrix_with_matrix, create_params_WbWAE, Word_by_Word_Attention_EntailmentPaper
...
def evaluate_lenet5(learning_rate=0.0002, n_epochs=2000, nkerns=[50,50,50], batch_size=1, window_width=3,
...
#     cost_tmp=T.fscalar()
    ...
    layer0_l_input = debug_print(embeddings[x_index_l.flatten()].reshape((maxSentLength, emb_size)).transpose(), 'layer0_l_input')
    layer0_r_input = debug_print(embeddings[x_index_r.flatten()].reshape((maxSentLength, emb_size)).transpose(), 'layer0_r_input')
...
#     
    U1, W1, b1=create_GRU_para(rng, emb_size, nkerns[0])
    layer1_para=[U1, W1, b1] 
 
    layer1_A1=GRU_Matrix_Input(X=layer0_l_input[:,left_l:-right_l], word_dim=emb_size, hidden_dim=nkerns[0],U=U1,W=W1,b=b1,bptt_truncate=-1)
    layer1_A2=GRU_Matrix_Input(X=layer0_r_input[:,left_r:-right_r], word_dim=emb_size, hidden_dim=nkerns[0],U=U1,W=W1,b=b1,bptt_truncate=-1)
# 
...
#     vec_r=debug_print(layer1_A2.output_vector_last.reshape((1, nkerns[1])), 'vec_r')
    ...
    W_a, w_a=create_params_WbWAE(nkerns[0], nkerns[1])
    W_by_W_attentionlayer=Word_by_Word_Attention_EntailmentPaper(l_hidden_M=layer1_A1.output_matrix, r_hidden_M=layer1_A2.output_matrix, 
                                                                 W_y=W_a[0], W_h=W_a[1], W_r=W_a[2], w=w_a, W_t=W_a[3], W_p=W_a[4], W_x=W_a[5], r_dim=nkerns[1])
    W_by_W_params=[W_a, w_a]
...
#     '''
    ...
    layer3_input=T.concatenate([#vec_l, vec_r,
                                W_by_W_attentionlayer.output.reshape((1, nkerns[1]))
...
#                                 cosine_sent, eucli_sent,
    ...
    layer3=LogisticRegression(rng, input=layer3_input, n_in=nkerns[1], n_out=3)

    
    #L2_reg =(layer3.W** 2).sum()+(layer2.W** 2).sum()+(layer1.W** 2).sum()+(conv_W** 2).sum()
    L2_reg =debug_print((layer3.W** 2).sum()+(U1** 2).sum()+(W1** 2).sum()+(W_a**2).sum()+(w_a**2).sum(), 'L2_reg')#
    ...
    params = layer3.params+ layer1_para+W_by_W_params#+[embeddings]# + layer1.params 
